  ...
  describe('createMultiple() method', () => {
    describe('should generate valid query', () => {
      test('when check with one search column', async () => {
        ...
        expect(mockQueryMethod.mock.calls[1][0]).toEqual("SELECT * FROM public.test WHERE foo ILIKE '%' ||  $1 LIMIT 8;");
        ...
        expect(mockQueryMethod.mock.calls[2][1]).toEqual(['foo_name']);
        ...
        expect(mockQueryMethod.mock.calls[4][0]).toEqual(
      ...
      test('when check with several search columns', async () => {
        ...
        expect(mockQueryMethod.mock.calls[0][0]).toEqual(
          "SELECT * FROM public.test WHERE foo ILIKE $1 || '%'  AND bar ILIKE $2 || '%' ;",
        );
        expect(mockQueryMethod.mock.calls[1][0]).toEqual(
          "SELECT * FROM public.test WHERE foo ILIKE '%' ||  $1 AND bar ILIKE '%' ||  $2 LIMIT 8;",
        );
        expect(mockQueryMethod.mock.calls[2][0]).toEqual(
          "SELECT * FROM public.test WHERE foo ILIKE '%' ||  $1 || '%'  AND bar ILIKE '%' ||  $2 || '%'  LIMIT 8;",
        );

        expect(mockQueryMethod.mock.calls[0][1]).toEqual(['foo_name', 'foo_name']);
        expect(mockQueryMethod.mock.calls[1][1]).toEqual(['foo_name', 'foo_name']);
        expect(mockQueryMethod.mock.calls[2][1]).toEqual(['foo_name', 'foo_name']);
        /*expect(mockQueryMethod.mock.calls[3][0]).toEqual(
      ...
      test('when there are LIMIT options', async () => {
        ...
        expect(mockQueryMethod.mock.calls[0][0]).toEqual("SELECT * FROM public.test WHERE foo ILIKE $1 || '%' ;");
        expect(mockQueryMethod.mock.calls[1][0]).toEqual("SELECT * FROM public.test WHERE foo ILIKE '%' ||  $1 LIMIT 6;");
        expect(mockQueryMethod.mock.calls[2][0]).toEqual(
